...
export const isSubscribable =
    <T>(source: unknown): source is Subscribable<T> => {
      if (typeof source === 'object' && source)
...
export const isObservableInput =
    <T>(source: unknown): source is ObservableInput<T> => {
      return isArrayLike(source) || isSubscribable(source) || isPromise(source);
    };

export const isAsyncFunction = <T>(source: unknown):
    source is AsyncFunction<T> => { return typeof source === 'function';};
