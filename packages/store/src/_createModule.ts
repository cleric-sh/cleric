...
export function createModule<TState, TSourceArgs extends SourceArgs>(
    name: string) {
  return <TSinkArgs extends SinkArgs = {}>(
    ...
    return (sources: TSourceArgs): MountableModule<TState, TSinkArgs> => (
      ...
      const props =
          Object.assign(sourceProps, sinkProps) as SourceProps<TSourceArgs>&
          SinkProps<TSinkArgs>;

      const effects = spec.effects ? spec.effects(props, slice) : {};

      const subscriptions = Object.getOwnPropertyNames(effects).map(
          name => { return effects[name].subscribe(); });

      if (spec.reducer) {
        const reducer = spec.reducer(sourceProps, slice);
        const reducerObservables =
            convertReducerArgsToObservables(slice, reducer);
