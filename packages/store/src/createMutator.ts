...
class MutatorNode<T> implements IMutationApi<T> {
  ...
  constructor(protected mutations: Mutation[], parent?: MutatorNode<any>,
              name?: string) {
...
const createMutatorProxy =
    <T>(mutations: Mutation[], mutator: MutatorNode<T>): Mutator<T> => {
      return (new Proxy(mutator, {
                get : (target, key, receiver) => {
                  if (!Reflect.has(target, key)) {
                    const name = key.toString();
                    const proxy = createMutatorProxy(
                        mutations, new MutatorNode(mutations, mutator, name));
