...
export type UnionApi<TConfigKey extends ConfigKey, T extends t.Any> =
    T extends t.UnionType<infer TCS>
    ? {
  $is: <TSubType extends Tuple.UnionOf<TCS>>(type: TSubType) =>
      Slice<TConfigKey, TSubType>;
}
  : never;
  ...
  export const UnionApi =
      createApi('Union', isUnionType, (configKey, type, slice) => {
        ...
        slice['$is'] = (guard: t.Any) => {
          ...
          if (index < 0)
            throw `Don't recognise this type...`;
